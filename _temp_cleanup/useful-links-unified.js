...
async function initializeUsefulLinksTable() {
  try {
    await query(`
    ...
    // Inserir links padrão se não existirem
    const existingLinks = await query('SELECT COUNT(*) as count FROM useful_links');
    if (existingLinks.rows[0].count === 0) {
      const defaultLinks = [
        ...
        {
          title: 'Documentação',
          description: 'Guias e tutoriais da plataforma',
          url: '/docs',
      ...
      for (const link of defaultLinks) {
        await query(`
          ...
          VALUES (?, ?, ?, ?, ?, ?)
        `, [link.title, link.description, link.url, link.category, link.icon, link.order_index]);
      }
      console.log('✅ Links úteis padrão criados');
    }
  } catch (err) {
    console.error('❌ Erro ao inicializar tabela useful_links:', err);
  }
}

// Inicializar ao carregar o módulo
initializeUsefulLinksTable();
...
router.post('/admin', requireAdmin, async (req, res) => {
  try {
    ...
    // Buscar o último link inserido (método compatível com SQLite)
